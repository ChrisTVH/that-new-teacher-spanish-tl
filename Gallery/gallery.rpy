# TODO: Translation updated at 2025-03-10 23:42

translate spanish strings:

    # game/Gallery/gallery.rpy:36
    old "Detention Duty"
    new "Servicio de Detención"

    # game/Gallery/gallery.rpy:36
    old "Embarrassing Act"
    new "Acto Vergonzoso"

    # game/Gallery/gallery.rpy:36
    old "Stuck Daphne"
    new "Daphne Atascada"

    # game/Gallery/gallery.rpy:36
    old "Watchers Eye"
    new "Ojo Observador"

    # game/Gallery/gallery.rpy:36
    old "King Of Clubs"
    new "Rey de los Clubes"

    # game/Gallery/gallery.rpy:36
    old "Swimming Lesson"
    new "Lección de Natación"

    # game/Gallery/gallery.rpy:36
    old "Morgana's A Slut"
    new "Morgana Es Una Puta"

    # game/Gallery/gallery.rpy:36
    old "Squeaky Clean"
    new "Completamente Limpio"

    # game/Gallery/gallery.rpy:36
    old "My Sleep Antics"
    new "Mis Travesuras Matutinas"

    # game/Gallery/gallery.rpy:36
    old "What To Wear?"
    new "¿Qué debería ponerse?"

    # game/Gallery/gallery.rpy:36
    old "69 Shades Of White"
    new "69 Tonos De Blanco"

    # game/Gallery/gallery.rpy:36
    old "Return To Sender"
    new "Devolver al Remitente"

    # game/Gallery/gallery.rpy:36
    old "Who's The Victor?"
    new "¿Quién es el Vencedor?"

    # game/Gallery/gallery.rpy:36
    old "\"Shay\""
    new "\"Shay\""

    # game/Gallery/gallery.rpy:36
    old "That New Yoga Instructor"
    new "Ese Nuevo Instructor de Yoga"

    # game/Gallery/gallery.rpy:36
    old "The First Punishment"
    new "El Primer Castigo"

    # game/Gallery/gallery.rpy:36
    old "Public Humiliation"
    new "Humillación Pública"

    # game/Gallery/gallery.rpy:36
    old "Mrs. Jacksons Climax"
    new "El Clímax de la Sra. Jackson"

    # game/Gallery/gallery.rpy:36
    old "Red Cheeks"
    new "Mejillas Rojas"

    # game/Gallery/gallery.rpy:36
    old "Erotic Fanfiction"
    new "Fantasía Erótica"

    # game/Gallery/gallery.rpy:36
    old "Lustful Visions"
    new "Visiones Lujuriosas"

    # game/Gallery/gallery.rpy:36
    old "Punishment Platform"
    new "Plataforma de Castigo"

    # game/Gallery/gallery.rpy:36
    old "Spikes & Spanks"
    new "Golpes y Azotes"

    # game/Gallery/gallery.rpy:163
    old "{size=30} Locked. {/size}"
    new "{size=30} Bloqueado. {/size}"

    # game/Gallery/gallery.rpy:191
    old "Photo Gallery"
    new "Galería de Fotos"

    # game/Gallery/gallery.rpy:204
    old "Unlock Gallery"
    new "Desbloquear Galería"

    # game/Gallery/gallery.rpy:229
    old "{size=+20}Replayable Scenes{/size}"
    new "{size=+20}Escenas Repetibles{/size}"

    # game/Gallery/gallery.rpy:230
    old "[SCu]/[SCl]"
    new "[SCu]/[SCl]"

# TODO: Translation updated at 2025-03-11 11:15

translate spanish strings:

    # game/Gallery/gallery.rpy:42
    old "Slippery When Wet"
    new "Resbaladizo y Húmedo"

    # game/Gallery/gallery.rpy:42
    old "Refill?"
    new "¿Rellenar?"

